...
**文件编码规则（适用于所有写文件动作，含四份文档与开发阶段的代码文件）：**

> **优先级**：语言规范技能（ts-lang-spec / c-lang-spec 等）若对编码有明确规定，则**以语言规范技能为准**；若无，则按下述默认规则兜底。无论何种情况，下方「修改已有文件」一条为**硬规则，任何规范不得覆盖**。

- **新建文件**（默认）：UTF-8 无 BOM、LF 换行——跨平台一致，且避免 BOM 被 Markdown 解析器误当作正文。语言规范技能另有要求时从其规定。
- **修改已有文件**（硬规则）：**必须保持原文件的编码格式和换行符不变**。写回前先识别原编码（如 GB2312/GBK/UTF-8/UTF-8 with BOM），按原编码原样写回，**绝不可擅自转换编码**。例如原文件是 GB2312，修改后仍须是 GB2312——转成 UTF-8 会造成乱码或与项目内其它文件编码不一致。
...
开发阶段编写代码时，必须遵循上述语言规范技能中定义的编码风格、命名约定、注释规范等要求。开发执行者应在开始编码前自动调用该技能，并严格遵守上述文件编码规则。
```
...
- **文件编码规则必须写入**——「编码规范」小节中必须包含文件编码要求（新建 UTF-8 无 BOM / 修改已有文件保持原编码不变），确保开发执行者读到
...
- [ ] 文件编码未被破坏：新建文件编码符合 plan.md「编码规范」小节的声明（语言规范技能优先），修改的已有文件保持原编码不变（验证：用编码检测工具核对，无乱码）
...
- **统一文件编码**——语言规范技能有规定时从其规定，否则默认新建文件 UTF-8 无 BOM、LF 换行；**修改已有文件须保持原编码不变**（如原为 GB2312 则仍按 GB2312 写回，绝不转换），此为硬规则，任何规范不得覆盖
